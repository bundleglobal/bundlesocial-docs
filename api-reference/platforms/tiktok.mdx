...
*   **Photo Mode (Images):** 1-10 images. **JPG/JPEG only.** Max **20 MB per image**. If you're thinking "surely PNG works too" - it doesn't.
...
TikTok only accepts **JPG** and **JPEG** files for Photo Mode posts. PNG, WebP, and everything else will be rejected. Convert before uploading. Each image must be under **20 MB**.

### Image sizing (Photo Mode)

For **IMAGE** posts, if photos are larger than **1920×1080** (or unusually tall/wide relative to that canvas), we may **auto-scale** them server-side so delivery matches what TikTok expects. You do not need to pre-shrink assets in every case-we handle the boring resizing when it helps.
