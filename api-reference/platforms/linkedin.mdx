...
Video max size is **1 GB** (not 200 MB - that was an old API limitation). Documents must be PDF format, max 100 MB.
...
### Raw analytics and reactions

[Raw analytics](/api-reference/analytics#raw-analytics-data) for LinkedIn retain **reaction** breakdowns from the platform (per reaction type) where available. Our standard post schema still exposes a single aggregated likes/reactions-style metric; use raw data when you need the full split.
