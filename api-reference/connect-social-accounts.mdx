...
**Facebook** and **Instagram** (when using the Facebook / Instagram Business login path) only. Optional boolean on **`POST /api/v1/social-account/connect`**. Defaults to `false` if you omit it-behavior matches how things worked before this option existed.

The **create portal link** endpoint does not take this flag; it is only on the **connect** body for custom integrations.
