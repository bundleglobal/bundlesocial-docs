...
| `disableAutoLogin` | No | Facebook, Instagram (Facebook method), and TikTok only. When `true`, bypasses the platform's automatic "you're already logged in" shortcut and forces the user to explicitly pick which account to connect. |
| `forceBrowserOAuth` | No | Instagram only. When `true`, direct Instagram connections on phones try to use browser OAuth instead of the Instagram app flow, avoiding the iOS deep-link issue. |
| `withBusinessScope` | No | Facebook and Instagram (Facebook login path) only. When you set this to `true` or `false`, the hosted connect page skips the business vs standard scope chooser and uses your value. When you omit it, users see the same chooser as on a normal connect flow. |
...
<Warning>
`disableAutoLogin` is designed for SaaS platforms where users manage many accounts from the same browser. Without this flag, the OAuth popup silently re-connects the last logged-in account, which is wrong when someone manages multiple clients. Enable it when your users connect accounts on behalf of others.
</Warning>
...
- **Instagram (FACEBOOK method):** Select an Instagram account connected to a Facebook Page.
- **Instagram (INSTAGRAM method):** No Facebook Page needed. Pass `instagramConnectionMethod: "INSTAGRAM"` in the connect body to use the direct Instagram OAuth flow.
...
Supported for **Facebook**, **Instagram (Facebook method)**, and **TikTok**.

When `true`, the OAuth URL includes a platform-specific flag that forces the user to explicitly confirm the account rather than silently re-using the last logged-in session:

<Warning>
Use this when your users connect accounts on behalf of clients or manage multiple accounts from the same browser. Without it, the OAuth popup re-connects whichever account was last logged in, with no chance to pick a different one.
</Warning>
...
When `true`, direct Instagram connections on phones try to open the browser OAuth flow instead of the Instagram app flow. This is useful when the Instagram iOS app deep-link flow gets stuck or returns users without completing authorization.

You can set it in both places:

1. **`POST /api/v1/social-account/connect`** (custom UI). Optional boolean.
2. **`POST /api/v1/social-account/create-portal-link`** (hosted UI). Optional boolean stored in the signed portal token.
...
### withBusinessScope (Facebook & Instagram)

**Facebook** and **Instagram** (when using the Facebook login path for Instagram) only.

You can set this flag in **both** places:

1. **`POST /api/v1/social-account/connect`** (custom UI). Optional boolean. If you omit it, it defaults to `false`, same as before this option existed.
2. **`POST /api/v1/social-account/create-portal-link`** (hosted UI). Optional boolean. It is stored in the signed portal token. The hosted `/connect` page reads it and passes it into the same connect logic as a custom client. If you omit it on the portal link, users still pick **business** vs **standard** on the hosted page.

| Value | What happens |
| :--- | :--- |
| `true` | Facebook **Business** login flow: OAuth uses the broader Meta scope set (includes `business_management`, `ads_management`, `ads_read` among the permissions we request). |
| `false` | **Standard** connection flow: lighter scope set, same as “classic” Facebook Page connections. |

<Warning>
Some **Facebook Pages** and **Instagram** accounts only appear in the page/account picker when the connection was made **with business scope**. If something is missing from the list, reconnect using the **other** method (`withBusinessScope` flipped) before assuming the picker is wrong. This usually comes down to Meta’s visibility rules, not a haunted UI.
</Warning>

If a connection goes bad, the social accounts screen offers **Reauthenticate** when our checks detect it-use that instead of disconnecting everything when you only need a fresh OAuth handshake.
...
| `instagram-direct-callback` | Error connecting Instagram (direct OAuth flow, `instagramConnectionMethod: "INSTAGRAM"`) |
...
| `instagram-not-professional-account` | Your Instagram account is not a Professional or Creator account |
...
1. The selected page/channel is cleared from the social account.
2. The OAuth connection stays intact - no re-auth needed.
...
1. Call `POST /social-account/unset-channel` to clear the current selection.
2. Call `POST /social-account/set-channel` with the new page/channel ID.
